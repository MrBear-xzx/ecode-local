...
- 二进制资源：按原始字节拉取、推送、删除和跨环境传递，替换失败时自动恢复远端旧版本。
- 跨环境变更集：将推送和生命周期记录整理为变更集，再应用到其他环境。
...
远端完整树中识别的资源目录参与普通同步和跨环境变更集。资源按原始字节校验，单文件上限为 100MB；替换失败时自动恢复远端旧版本。完整规则见[用户指南](docs/user-guide.md#二进制资源)。
...
- 根文件夹用绿色或灰色包图标表示发布状态，行内只显示前置加载顺序；App ID、发布状态和资源数量可在悬停说明中查看。状态无法确认时不提供发布或取消发布，内部文件夹不能单独发布。
- JS/CSS 文件可设置或取消前置加载；已前置加载文件显示 `P`，普通文件仍使用当前主题的原生图标和颜色。

发布、文件前置状态和顺序彼此独立；顺序越小越先执行，默认 `10000`。前置 JS/CSS 早于普通按需加载代码执行，使用前应确认影响范围。扩展会为实际改变且回读确认成功的生命周期操作保存历史记录，供跨环境变更集选择；侧边栏按目标路径展示记录，并支持复选批量删除历史。完整规则见[用户指南](docs/user-guide.md#前置加载与文件夹发布)。

“源码目录”先显示缓存再后台刷新，写操作仍使用实时状态。原生资源管理器不提供生命周期操作，只显示同步变更装饰：`A`、`M`、`D`、`!` 或 `↓`。

需要主动重读时，可从命令面板执行 `Ecode: 刷新源码结构与生命周期状态`。普通拉取和推送不会隐式改变发布、前置加载或顺序；只有变更集中明确包含的生命周期记录会在应用代码成功后对齐到目标环境。

## 跨环境发布

1. 在来源环境完成推送。
2. 执行 `Ecode: 创建跨环境变更集`，选择相关推送记录和生命周期记录；也可创建纯生命周期变更集。
3. 检查变更集中的文件、差异及前置加载、顺序和发布目标状态。
4. 切换到目标环境，执行“应用到当前环境”并确认。

变更集保存最终文件快照和生命周期期望状态，不执行分支或逐行合并。代码未全部成功时不会执行生命周期配置；前置状态或顺序失败时不会发布文件夹。Ecode 服务端不提供事务，遇到部分成功可在排除问题后重新应用，扩展会跳过已收敛项目并继续处理未完成内容。
...
- 二进制冲突的差异页显示本地、基线和远端的路径、扩展名、大小、SHA-256 与存在状态，不执行文本 Diff。
