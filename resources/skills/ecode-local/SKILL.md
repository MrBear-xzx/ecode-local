---
name: ecode-local
description: 通过插件生成的通用 CLI 查询、编辑及操作 Ecode Local 工作区，并使用生成的 API/组件知识辅助编码。通用编码 Agent 修改 Ecode 源码，或处理环境、同步、冲突、推送记录、变更集、开发文档和 AI 支持时使用。
---
...
5. `configure`、`addEnvironment`、`searchDocumentation` 和 `openOnlineDocumentation` 提交后返回 `pending`；等待用户完成 VS Code 表单或选择器，再使用返回的 `wait --request-id <ID>` 续查。其他 action 超时表示结果未知，也只能续查原请求，不得用新 ID 重试状态变更。
...
1. 状态变更前先调用 `getState` 或 `refreshChanges`。
...
6. 状态变更后重新查询；CLI 返回 `cancelled`、`rejected` 或 `failed` 不代表成功。
