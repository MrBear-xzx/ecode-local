...
export async function copyFileAtomic(file: string, source: string): Promise<void> {
  const target = path.resolve(file);
  const previous = writeQueues.get(target) ?? Promise.resolve();
  const current = previous.then(
    () => copyFileAtomicNow(target, source),
    () => copyFileAtomicNow(target, source),
  );
  writeQueues.set(target, current);
  try {
...
async function copyFileAtomicNow(file: string, source: string): Promise<void> {
  const temporary = `${file}.${process.pid}-${randomUUID()}.tmp`;
  await fs.mkdir(path.dirname(file), { recursive: true });
  let writeError: unknown;
  try {
